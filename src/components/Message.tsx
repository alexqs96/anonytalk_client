          ...
          <div className="my-2">
            {file.startsWith("data:image") ? (
              ...
              <a
                ...
                <FileIcon className="fill-current w-[.9em]" /> Descargar{" "}
                {file.startsWith("data:application/pdf") ? "PDF" : "Archivo"}
              </a>
            )}
          </div>
        )),
      ...
      <div
        ...
        {data.author.length === 0 ? null : (
          <>
            ...
            {data.socket === data.id ? (
              <span className="font-semibold opacity-70 text-[.7rem]">
                (tú)
              </span>
            ) : null}
            <span
              title={parseTime(data.time).long}
              className="text-[.7rem] opacity-70 hover:underline">
              {parseTime(data.time).short}
            </span>
          </>
        )}
      </div>
